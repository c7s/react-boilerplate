...
try {
  const msgPrefixMatch = /([A-Z]+-\d+)/.exec(
    execSync("git symbolic-ref --short HEAD", { encoding: "utf-8" })
  );
  ...
  if (msgPrefixMatch && !isAlreadyWithPrefix && !isMerge) {
    ...
    console.warn(
      `[WARN] You shouldn't specify prefix while being on task branch`
    );
  }
  ...
  console.warn(
    "[WARN] Something unusual is happening, commit message will not be prepared"
  );
}
